...
flowchart TB
    ...
    subgraph ds["DesignSystem"]
        dsC["PitHead, PitPose, UtilityLayer, TileCard, ScreenHeader, LoadFailureBanner, PitColor, DesignTokens"]
    end
...
stateDiagram-v2
    state "Utility layer: PitPresenceModel" as Utility {
        ...
        state "knock, accent eyes, two dips" as u_knock
...
drawable (dimmed eyes in `PitHead`) but no model shows it, as ADR 0028 records.
The head draws each state as one `PitPose`; only side gaze, startle and knock
tilt or lift it, and the knock's two bumps are dips from its 3 pt lift
([ADR 0039](../decisions/0039-pit-head-control.md)).
...
| Pit presence and motion | Eyes rest, blink and look around irregularly, yield to any activity, startle and knock to ask, think and glance in the sheet, close on leaving; every state a distinct static pose with inward eye tilt at most 6° (REQ-PIT-022); the eyes lit in the accent only on a knock (REQ-PIT-023); the head tilts or lifts only when thinking, startled or knocking (REQ-PIT-024); Reduce Motion mapping | 0012, 0019, 0028, 0039 | `Domain/Pit/PitPresence.swift`, `Features/Pit/PitPresenceModel.swift`, `PitCaptureEyes.swift`, `PitActivityReporting.swift`, `DesignSystem/Components/PitPose.swift`, `PitHead.swift`, `PitEyeLife.swift` | `PitPresenceTests`, `PitCaptureEyesTests`, `PitEyesGlyphTests`, `PitLeavingTests`, `PitActivityReportingTests`, `PitPoseTests`, `PitKnockColourTests`, `PitHeadMotionTests` | Unit tests; every pose rendered with `ImageRenderer` (RD-011); motion timing on screen not recorded here |
...
| Next-service widget | Small, Lock Screen rectangular and inline widget showing Service's first operation (name, status word, one fact) or a calm empty state; tap opens Service; reloads after saved commands and at the next time-based change; store moved once into the App Group container; the small widget draws an accentable eyebrow, the name, the shared `StatusChip` and the fact in PitColor and PitTypography roles on `surfaceSecondary`, the rectangular one the shared status glyph beside the word, the inline one system text (FU-2) | 0036, 0038 | `PitstopWidgets/NextServiceWidget.swift`, `Shared/NextServiceContent.swift`, `Shared/AppLink.swift`, `Shared/DesignSystem/`, `Infrastructure/Persistence/StoreRelocation.swift`, `NextServiceStoreReader.swift`, `Infrastructure/Widgets/` | `StoreRelocationTests`, `NextServiceWidgetTests`, `NextServiceWidgetSourceTests`, `WidgetEntryTests`, `DesignRulesTests` | Unit tests; a replica of the small and rectangular layouts rendered in light, dark, en, ru, uk and larger text sizes (FU-2); gallery, tinted and dark Home Screen, Lock Screen rendering and TestFlight upgrade device checks pending (DEV-WIDGET) |
