...
|---|---|---|---|---|---|
| Design system | Surface tiers: tinted stage, grouped lists, glass only for floating controls; status as word + glyph + colour; typography roles; Reduce Transparency and Increase Contrast fallbacks; source rules against colour literals in features and glass outside the design system | 0009, 0038 | `DesignSystem/PitColor.swift`, `PitTypography.swift`, `DesignTokens.swift`, `DesignSystem/Components/` (`StageSurface`, `StatusChip`, `StatusGlyph`, `EmptyState`, `GlassPill`, `StepStrip`, `RemainingShareTrack`, `PitGlass`, `ChipFlowLayout`) | `DesignRulesTests`, `PitColorTests`, `StatusGlyphTests`, `RemainingShareTrackTests`, `ChipFlowLayoutTests` | Unit tests; components previewed only, screens adopt them in RD-001…RD-012 |
