...
| Service | `MaintenanceEngine`, `MaintenanceContext`, `MaintenanceOperationState`, `ServicePlanner`, `RevokeMaintenanceCompletionCommand`, `StopTrackingOperationCommand`, `ServiceViewModel`, `ServiceView`, `OwnerInterval`, `TrackSeveralViewModel`, `TrackSeveralView` | Deterministic status from the owner's intervals and confirmed completions, suggested visit scope, track / track several / mark done / change interval / undo / stop tracking; "Track several" writes one `setMaintenancePolicy` (`userCustom`) per confirmed item and stores no car-type answer (CB-005, MNT-POL-001, MNT-PRE-001, ADR 0010, 0031, 0033) |
...
| Maintenance Progress | Progress toward next service | — | From completion + policy | `MaintenanceOperationState` |
