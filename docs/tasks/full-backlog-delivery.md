...
| SYS-005 | `main` (working tree) | committed (gallery checks pending) | ADR 0025 (owner decisions 2026-09-21): new `PitstopWidgets` app extension (`dev.vil4max.pitstop.widgets`, embedded through an `Embed App Extensions` copy phase, `MARKETING_VERSION` 1.1.0 and `CURRENT_PROJECT_VERSION` as the app, iOS 27.0, Swift 6, no entitlements, hand-written in `project.xcproj` after the OneCart shape) with an Open Pit control (`ControlWidgetButton(action: OpenPitIntent())`) and a data-free `systemSmall`/`accessoryCircular` widget opening `pitstop://pit` through `widgetURL`; `OpenPitIntent` adopts `OpenIntent` with a one-case `CaptureSurface` target (default `pit`; the App Shortcut keeps working, no hidden intent) and moves with `CaptureSurfaceRequests` into `Shared/` (member of both targets); intent strings in a shared `OpenPit` table compiled into both bundles; `pitstop` URL scheme in `Config/Info.plist`, `onOpenURL` on the scene root calls `CaptureSurfaceRequests.request(opening:)`, which accepts only `pitstop://pit`, so ADR 0024 deferral applies. Tests `WidgetEntryTests` (URL allow-list, deferral, registered scheme, embedded appex bundle ID and versions, OpenIntent metadata in both bundles, string tables) and an `AppShortcutsTests` target-default check (ADR-0025, REQ-CAPTURE-023). `just verify` passed with Xcode 27.2 (`verify OK (DoD)`); the scheme also builds with Xcode 27.0 and embeds `PitstopWidgets.appex`; `project_versions.py` prints 1.1.0; no `project.pbxproj`. Simulator on `claude-Pitstop-1b8d76c5`: `pitstop://pit` opens the Pit sheet from a terminated app and over Service with the app warm; `pitstop://settings` opens nothing. Widget gallery, Control Center, Lock Screen and Action button not exercised (gestures; device checks with SYS-006); independent review pending |
