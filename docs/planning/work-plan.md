...
**Next task:** RD-012 (`next`); the round is tracked in [`../tasks/redesign-ios27.md`](../tasks/redesign-ios27.md)  
...
| DEV-WIDGET | Deferred, device-only (owner, 2026-09-24): checked on the TestFlight 1.2.0 build after RD-012. Widget gallery (capture and next-service widgets), the redesigned "Remember" widget's tinted and dark Home Screen appearances (RD-009: the whole glyph disc takes the tint, the disc and text stay legible in dark; not checked on a device), the restyled "Next service" widget's tinted and dark Home Screen appearances and its Lock Screen rectangular rendering (FU-2: the eyebrow takes the tint, the chip and its glyph stay legible in the default group and in dark, the status glyph reads by shape in the vibrant Lock Screen tint; not checked on a device), Control Center, Lock Screen control and rectangular/inline widget, Action button, Shortcuts listing; TestFlight upgrade check of the store move; first Xcode Cloud archive carries the App Group in both profiles | ADR 0025, ADR 0026, ADR 0036 |
| DEV-PIT-SHEET | Deferred, device-only (owner, 2026-09-24): checked on the TestFlight 1.2.0 build after RD-012. On-screen check of Pit inside sheets, pending since RD-010 (the checks need taps or the system link prompt, so none was seen): Pit at the bottom-trailing spot in each feature sheet (car editor, note, history event, planned date, Track an operation, Track several, Mark as done, dashboard reading) and in Settings, at the medium and large detents; above the keyboard, covering neither a sheet's trailing controls nor a keyboard accessory, at AX sizes and in landscape; shown disabled while Track several, the planned date editor or the dashboard reading saves; tapping him opens capture over the sheet and closing it returns to the sheet with its input | REQ-UTILITY-012, REQ-PIT-026 |
